...
def render_sidebar():
    with st.sidebar:
        ...
        st.warning("⚠️ API 并发限制较高，已强制使用顺序模式防止 429 报错")
        st.divider()
...
def run_analysis(idea: str):
    ...
    for emoji, name, agent, key in agents_info:
        try:
            ...
            st.session_state.results[name] = f"⚠️ 分析失败: {e}"
            st.session_state.completed_agents.append(name)

    st.session_state.execution_time = time.time() - start_time
    filepath = save_to_markdown(st.session_state.context, idea)
    st.session_state.saved_filepath = filepath

def save_to_markdown(context, idea):
    output_dir = Path(__file__).parent.parent.parent / "output"
    output_dir.mkdir(exist_ok=True)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    safe_idea = idea.replace("/", "_").replace(" ", "_")[:15]
    filepath = output_dir / f"{safe_idea}_{timestamp}.md"
    sections = [
        ("📊 市场定位", context.get("market_analysis", "")),
        ("📦 产品定义", context.get("product_plan", "")),
        ("💰 商业模式", context.get("business_model", "")),
        ("📈 财务规划", context.get("finance_plan", "")),
        ("⚠️ 风险评估", context.get("risk_analysis", "")),
        ("🎨 品牌设计", context.get("design_strategy", "")),
        ("🎬 流量脚本", context.get("marketing_script", "")),
        ("🔍 SEO 策略", context.get("seo_strategy", "")),
    ]
    lines = [f"# 🚀 创业方案: {idea}", "", f"生成时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", ""]
...
def render_right_panel():
    ...
    features = [
        ("🤖", "8 个 AI Agent", "全方位商业分析与规划"),
        ("⚡", "自动顺序执行", "避免 API 并发限制导致报错"),
        ("💰", "成本透明", "实时追踪 Token 消耗"),
        ("📄", "一键导出", "Markdown 与 PDF 格式"),
    ]
...
def main():
    st.set_page_config(page_title="BizGenesis", page_icon="🚀", layout="wide")
    init_session_state()
    render_header()
    render_sidebar()
    ...
    if start_btn and idea:
        with st.spinner("🤖 AI 团队正在分析中，请耐心等待（约 10-15 分钟）"):
            run_analysis(idea)
        st.rerun()
